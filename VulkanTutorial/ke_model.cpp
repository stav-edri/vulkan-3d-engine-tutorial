...
namespace ke {
	KeModel::KeModel(KeDevice& device, const std::vector<Vertex>& vertices) : keDevice{device}
	{
	...
	void KeModel::createVertexBuffers(const std::vector<Vertex>& vertices)
	{
		...
		void* data;
		vkMapMemory(keDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
		memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
		vkUnmapMemory(keDevice.device(), vertexBufferMemory);
